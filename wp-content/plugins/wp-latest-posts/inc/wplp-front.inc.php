...
class WPLPFront
{
    ...
    private function resetSettingsPremium()
    {
        ...
        if ($this->widget->settings['theme'] === 'masonry-category') {
            ...
            $bottom_box = array('Date', 'Read more');
        } elseif ($this->widget->settings['theme'] === 'material-horizontal') {
            $bottom_box = array(
                $this->widget->settings['dfDate'],
                'Read more'
    ...
    private function themeMaterialHorizontal($fields, $post_id, $num)
    {
        ...
        $html_img = '<div class="wplp-mh-post-image-column wplp-mh-flex-column-100">
                    <figure class="wplp-mh-post-image">
                        <a href="'.$permalink.'" '.$target.' title="'.$title_text.'" class="js-lazy-parent lazy-loaded" tabindex="-1">
                            <div class="wplp-mh-image wplp-mh-image-responsive wplp-mh-image-landscape js-lazy lazy-loaded" style="background: '.$img.'"></div>
        ...
        $html_des = '<div class="wplp-mh-post-content-column wplp-mh-flex-column-100">
                    <div class="wplp-mh-post-container">
                        <header class="wplp-mh-post-header">
                            <div class="wplp-mh-flex-container wplp-mh-flex-container-center wplp-mh-flex-gutter-small">
                                <div class="wplp-mh-flex-column-auto wplp-mh-pd-0">
                                    <div class="wplp-mh-post-terms wplp-mh-post-categories">'.$category.'</div>
                                </div>
                                <div class="wplp-mh-flex-column-auto">
                        ...
                        <div class="wplp-mh-post-excerpt" itemprop="description">
                            '.$content.'
                        </div>
                            ...
                            <div class="wplp-mh-flex-container wplp-mh-flex-container-center wplp-mh-flex-gutter-small">
                                <div class="wplp-mh-flex-column-auto wplp-mh-flex-grow-2 wplp-mh-pd-0">
                                    <div class="wplp-mh-post  -meta">
                                        <ul>
                                            <li class="wplp-mh-post-meta-date">
                                                <a href="'.$permalink.'" '.$target.'>
                                                    <time class="published" itemprop="datePublished" datetime="2018-01-01T18:00:58+00:00"> '.$date.' </time>
                                                </a>
                                            </li>
                                        </ul>
                                    </div>
        ...
        if ($num % 2 === 0) {
            $html .= $html_des.$html_img;
        } else {
            $html .= $html_img.$html_des;
        }
        $html .= '</div></article>';
    ...
    private function boxMisc($box_name)
    {
        ...
        if (is_multisite()) {
            if ('src_category_list' === $this->widget->settings['source_type']) {
                switch_to_blog((int) $post->curent_blog_id);
                $before .= '<a href="' . get_term_link($post->ID) . '" ' . $target . ' class="'.$class_tag_a.'" aria-label="'.get_the_title($post->ID).'">';
                restore_current_blog();
            } else {
                $before .= '<a href="' . get_blog_permalink($post->curent_blog_id, $post->ID) . '" ' . $target . ' class="'.$class_tag_a.'" aria-label="'.get_the_title($post->ID).'">';
            }
        } else {
            if ('src_category_list' === $this->widget->settings['source_type']) {
                ...
                $before .= '<a href="' . $links . '" ' . $target . ' class="'.$class_tag_a.'" aria-label="'.get_the_title($post->ID).'">';
            } else {
                $before .= '<a href="' . get_permalink() . '" ' . $target . ' class="'.$class_tag_a.'" aria-label="'.get_the_title($post->ID).'">';
            }
        }
